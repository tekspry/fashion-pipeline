$root = "c:\pocs\FastMCP\FashionAccessoryPipeline\src"
...
foreach ($s in $services) {
    ...
    Start-Process -FilePath $exePath -ArgumentList "--urls $($s.Url) --environment Development" -WorkingDirectory $projectDir -NoNewWindow
    Start-Sleep -Seconds 1
}

Write-Host "All background services started."
